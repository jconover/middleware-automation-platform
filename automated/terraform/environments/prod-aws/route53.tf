...
resource "aws_cloudwatch_metric_alarm" "route53_health_check" {
  ...
  # Use the SLO alerts SNS topic if ECS is enabled, otherwise create dedicated topic
  alarm_actions = var.ecs_enabled ? [aws_sns_topic.slo_alerts[0].arn] : [aws_sns_topic.route53_alerts[0].arn]
  ok_actions    = var.ecs_enabled ? [aws_sns_topic.slo_alerts[0].arn] : [aws_sns_topic.route53_alerts[0].arn]
...
# SNS Topic for Route53 alerts (when ECS is not enabled)
resource "aws_sns_topic" "route53_alerts" {
  count = local.route53_enabled && !var.ecs_enabled ? 1 : 0
...
resource "aws_s3_object" "maintenance_index" {
  ...
  content      = <<-HTML
    ...
    <head>
        ...
        <title>Scheduled Maintenance - ${var.project_name}</title>
        <style>
            * {
                margin: 0;
                padding: 0;
...
resource "aws_s3_object" "maintenance_error" {
  ...
  content      = <<-HTML
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Service Unavailable - ${var.project_name}</title>
        <style>
            body {
...
# =============================================================================
# Health Check for Latency-Based Routing (Optional)
# =============================================================================
# Additional health checks for multi-region deployments

resource "aws_route53_health_check" "alb_calculated" {
  count = local.route53_enabled && var.enable_calculated_health_check ? 1 : 0

  type                   = "CALCULATED"
  child_health_threshold = 1

  child_healthchecks = [
    aws_route53_health_check.alb_primary[0].id
  ]

  tags = {
    Name        = "${local.name_prefix}-calculated-health-check"
    Environment = var.environment
    Purpose     = "Aggregated Health Status"
  }
}
...
# =============================================================================
# Provides health check latency metrics for monitoring dashboards
...
resource "aws_cloudwatch_metric_alarm" "route53_latency_high" {
  ...
  alarm_actions = var.ecs_enabled ? [aws_sns_topic.slo_alerts[0].arn] : (
    local.route53_enabled && !var.ecs_enabled ? [aws_sns_topic.route53_alerts[0].arn] : []
  )
